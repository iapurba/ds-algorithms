...
class MaxHeap:
    ...
    def _heapify_down(self, idx=0):
        ...
        while (self._has_left_child(idx)):
            greater_child_idx = self._get_left_child_idx(idx)

            # If right child exists and less than the left child,
            # update the greater child index to right child's index
            if (self._has_right_child(idx) and self._right_child(idx) > self._left_child(idx)):
                greater_child_idx = self._get_right_child_idx(idx)

            # Swap if the parent is greater than the greater child
            if self.heap[idx] > self.heap[greater_child_idx]:
                self._swap(idx, greater_child_idx)
            else:
                break

            # Update index to greater child's index
            idx = greater_child_idx
